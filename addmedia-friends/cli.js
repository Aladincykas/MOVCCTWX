#!/usr/bin/env node
/**
 * CC Media Uploader — Friends Edition — same unified music + video uploader
 * as addmedia/, for handing out to people you trust to ADD to your Komanda X
 * library but not necessarily to remove things from it. Deliberately has NO
 * delete flow in the menus at all -- deleteSongFlow/deleteVideoFlow/
 * deleteFile() from addmedia/cli.js were removed here entirely, not just
 * hidden, so there's no menu path to them.
 *
 * IMPORTANT, READ BEFORE HANDING THIS OUT: this only prevents ACCIDENTAL
 * deletion through normal use of this specific tool. GitHub's Contents API
 * doesn't offer an "add but not delete" permission scope -- a token with
 * write access to a repo (which this tool needs, to add anything at all)
 * can ALSO delete through that exact same token, via the GitHub website,
 * git directly, curl, or any other tool -- this program choosing not to
 * expose a delete button doesn't and can't stop that. If you genuinely
 * don't trust someone with delete access, the only real protection is to
 * not give them a token with write access to your repo in the first place
 * (e.g. have them send you videos/songs to add yourself instead).
 *
...
async function firstTimeSetup() {
    ...
    console.log("\nNOTE: this token can technically still delete files too (GitHub doesn't offer");
    console.log("an add-only permission) -- this tool just never shows you a way to. Don't share");
    console.log("this token anywhere else if you don't want that.\n");
...
async function videoCategory(config) {
    ...
    const ctx = { owner: config.owner, token: config.token, repo: repoConfig.repo, branch: repoConfig.branch };
    await addVideoFlow(ctx);
}

async function main() {
    console.log("=== CC Media Uploader -- Friends Edition (Komanda X) ===");
    console.log("Add-only: there is no delete option in this tool.\n");
