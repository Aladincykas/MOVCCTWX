...
// Drop a video file you already own here (movies you can't fetch from
// YouTube -- your own rips, purchased downloads, etc) and Video mode will
// list it as a pickable option instead of only ever going to YouTube.
// Auto-created on first run if it doesn't exist.
const LOCAL_VIDEOS_DIR = path.join(BASE_DIR, "videos_to_upload");
const LOCAL_VIDEO_EXTENSIONS = [".mp4", ".mkv", ".mov", ".avi", ".webm", ".m4v", ".wmv", ".flv"];
...
// A CC:Tweaked monitor redraw is a full-screen mon.blit PLUS up to 16
...
// especially on a large multi-block monitor. Originally confirmed in-game:
// playing a 30fps video made the physical monitor go black and stay
// corrupted even across a reboot, only recoverable by breaking and
// replacing the block -- a Minecraft/CC-side renderer overload, not a Lua
// bug -- which is why this was capped down to 10 at the time.
//
// Raised back up to 25 since -- videoplayer.lua's row-diffing (skip
// mon.blit for any row that's byte-for-byte identical to the previous
// frame) and the locked-palette fix (most of those 16 setPaletteColor
// calls get skipped too, only the few slots that actually change get
// touched) both shipped AFTER that original 30fps corruption was found,
// and cut the REAL per-frame data throughput a lot even at a higher fps --
// confirmed working again at 25 in-game since those landed. If it starts
// corrupting again at this fps, drop it back down.
const DEFAULT_FPS_CAP = 25;
...
async function convertVideoLocally(inputPath, workDir, baseName, opts) {
    ...
    run(FFMPEG_PATH, [
        ...
        // Confirmed in-game: video audio comes out noticeably quieter than
        // music, even though speaker.playAudio() is called identically for
        // both (same volume value, same speakers) -- so the gap isn't in
        // playback at all, it's baked into the conversion. Music's audio
        // goes through this project's OWN dfpwm encoder (a plain npm
        // package, fed raw PCM this code controls directly), but video's
        // audio goes through sanjuuni.exe's OWN internal DFPWM encoder (a
        // separate third-party implementation, given this AAC-encoded
        // segment as input) -- there's no way to inspect or directly
        // control whatever gain/headroom choices sanjuuni makes internally.
        // A flat +6dB boost here, BEFORE sanjuuni ever sees the audio, is a
        // safe, predictable way to compensate regardless of what's actually
        // happening on sanjuuni's side -- a fixed gain (not adaptive
        // loudness normalization) on purpose, since normalizing each
        // ~90s segment independently could make volume noticeably jump
        // between chunks of the SAME video, which would be a worse
        // listening experience than being uniformly quiet.
        "-af", "volume=6dB",
        "-c:a", "aac", "-map", "0",
...
// ==== Video ====
function listLocalVideoFiles() {
    if (!fs.existsSync(LOCAL_VIDEOS_DIR)) return [];
    return fs.readdirSync(LOCAL_VIDEOS_DIR)
        .filter((f) => LOCAL_VIDEO_EXTENSIONS.includes(path.extname(f).toLowerCase()))
        .sort();
}

async function addVideoFlow(ctx) {
    if (!fs.existsSync(LOCAL_VIDEOS_DIR)) fs.mkdirSync(LOCAL_VIDEOS_DIR, { recursive: true });
    const localFiles = listLocalVideoFiles();

    // tmpVideo/detectedTitle/isOwnFile get filled in by whichever branch
    // runs below -- isOwnFile matters at cleanup time: a file YOU dropped
    // in videos_to_upload/ must NEVER be deleted afterward, only a
    // yt-dlp download to os.tmpdir() should be.
    let tmpVideo, detectedTitle, isOwnFile = false;

    if (localFiles.length > 0) {
        console.log(`\nFound ${localFiles.length} file(s) in ${LOCAL_VIDEOS_DIR}:`);
        localFiles.forEach((f, i) => console.log(`  ${i + 1}. ${f}`));
        console.log(`  0. None of these -- fetch from YouTube instead`);
        const choice = (await ask("Choose a file (or 0 for YouTube): ")).trim();
        const idx = parseInt(choice, 10);
        if (idx >= 1 && idx <= localFiles.length) {
            const chosenFile = localFiles[idx - 1];
            tmpVideo = path.join(LOCAL_VIDEOS_DIR, chosenFile);
            detectedTitle = path.parse(chosenFile).name;
            isOwnFile = true;
        }
    }

    if (!tmpVideo) {
        // Same source as addSongFlow -- a search term or a direct URL,
        // fetched with yt-dlp. Video mode never touches the music repos
        // regardless of where the source came from; that separation is
        // enforced by which repo `ctx` points at (chosen in
        // videoCategory() below), not by the download source.
        ...
        tmpVideo = path.join(os.tmpdir(), `addmedia_video_${Date.now()}.mp4`);
        ...
        const titleOut = run(YTDLP_PATH, ["--print", "title", ...jsRuntimeArgs, ...clientArgs, target]).trim();
        detectedTitle = titleOut || query;

        if (!fs.existsSync(tmpVideo)) throw new Error("Download failed: no output file produced.");
    }

    await ensureMusicTools(); // ffmpeg lives here too -- needed either way for conversion
    ...
    try {
        ...
        if (isOwnFile) console.log(`(Your original file in ${LOCAL_VIDEOS_DIR} was left alone -- delete it yourself whenever you're done with it.)`);
    } finally {
        fs.rmSync(workDir, { recursive: true, force: true });
        // Only ever clean up a yt-dlp temp download -- never a file the
        // user put in videos_to_upload/ themselves.
        if (!isOwnFile && fs.existsSync(tmpVideo)) fs.unlinkSync(tmpVideo);
    }
}
